...
type
  TRepForm = class(TForm)
    ...
    ADOQueryRepИтог: TCurrencyField;
    ADOQueryRepПоставкаПоставщик: TIntegerField;
    ADOQueryRepПоставщикПоставщик: TWideStringField;
    ADOQueryRepПоставкаТовар: TIntegerField;
    ADOQueryRepТоварТовар: TWideStringField;
    ADOQueryRepТиптовара: TWideStringField;
    ADOQueryRepЦеназа1ед: TBCDField;
    ADOQueryRepВес: TFloatField;
...
begin
  if (DataSet.FieldByName(Cost).Value <> null) and (DataSet.FieldByName(Weight).Value <> null) then
    DataSet['Итог'] := StrToInt(DataSet.FieldByName(Cost).Value) *
