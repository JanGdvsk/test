...
begin
  ...
  begin
    ...
    GlobalQuery.Edit;
    if XchngForm.Caption = 'Добавить' then
  ...
  //if localQuery.Modified then   костыль 3 ~ много пустых записей
  LocalQuery.Append;
  // Костыль 2
...
begin
  With DeliveriesForm do
  begin
    LocalQuery := QSimpleDelivers;             //в зависимости от контекста
    GlobalQuery := AdoQueryDelivers;
...
begin
  With DeliveriesForm do
  begin                                        //Передача значений с формы в БД
    LocalQuery := QSimpleDelivers;             //в зависимости от контекста
    GlobalQuery := AdoQueryDelivers;
    if MainGrid.DataSource = DataSourcePrice then
    begin
...
begin
  ...
  begin
    DeliveriesForm.StartQuery(QPriceCost, 'Select * From'
      +' Прайс Where (Поставщик = ' + InttoStr(DblookUpCombobox1.KeyValue)
      + ' ) and (Товар = ' + IntToStr(DblookUpCombobox3.KeyValue)+ ' )','' );
